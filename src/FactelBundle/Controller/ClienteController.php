...
class ClienteController extends Controller {
     ...
     *
     * @Route("/", name="cliente")
     * @Secure(roles="ROLE_EMISOR, ROLE_ADMIN")
     * @Method("GET")
     * @Template()
     */
    public function indexAction() {
        ...
        if ($this->get("security.context")->isGranted("ROLE_ADMIN")) {
            $entities = $em->getRepository('FactelBundle:Cliente')->findAll();
        } else {
            $emisorId = $this->get("security.context")->gettoken()->getuser()->getEmisor()->getId();
            $entities = $em->getRepository('FactelBundle:Cliente')->findBy(array('emisor' => $emisorId));
        }
        $deleteForms = array();
        ...
        if ($form->isValid()) {
            ...
            for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) {
                if (isset($data->sheets[0]['cells'][$i][1]) && isset($data->sheets[0]['cells'][$i][2]) && isset($data->sheets[0]['cells'][$i][3])) {
                    $identificacion = $data->sheets[0]['cells'][$i][3];
