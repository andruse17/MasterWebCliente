...
class ProductoController extends Controller {
    ...
    public function productosAction() {
        ...
        foreach ($entities as $entity) {
            ...
            $clienteArray[$i] = [$entity->getId(), $entity->getCodigoPrincipal(), $entity->getCodigoAuxiliar(), $entity->getNombre(), $entity->getPrecioUnitario(), $iva, $ice, $IRBPNR, $tarifa];
            $i++;
        }
        ...
        if ($form->isValid()) {
            ...
            for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) {
                ...
                if (isset($data->sheets[0]['cells'][$i][1]) && isset($data->sheets[0]['cells'][$i][2]) && isset($data->sheets[0]['cells'][$i][4]) && isset($data->sheets[0]['cells'][$i][5])) {
                    $codigoPrincipal = $data->sheets[0]['cells'][$i][2];
                    ...
                    $producto->setNombre($data->sheets[0]['cells'][$i][1]);
                    $producto->setCodigoPrincipal($data->sheets[0]['cells'][$i][2]);
                    if (isset($data->sheets[0]['cells'][$i][3])) {
                        $producto->setCodigoAuxiliar($data->sheets[0]['cells'][$i][3]);
                    }
